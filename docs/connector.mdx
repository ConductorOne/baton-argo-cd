...
- **Delete** (account deprovisioning) is permanent. It performs four steps:
  ...
  2. Removes the account's role grants from `policy.csv` in `argocd-rbac-cm`, so an account created
     later with the same name does not inherit them. Policy (`p`) lines that name the account are
     left in place, and rewriting `policy.csv` drops its `#` comment lines.
