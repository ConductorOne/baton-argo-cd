...
- Account names may contain only alphanumerics, `-` and `_`. ArgoCD splits `accounts.*` keys in
  ...
  is parsed as a `smith` property of an account named `john`. Provisioning and deprovisioning
  both reject such names rather than writing a key ArgoCD would silently ignore.
